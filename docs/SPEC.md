...
| F-07 | 촬영 피드백: 짧은 진동 + 토스트. 셔터음(`MediaActionSound`)은 `Settings.PLAY_SHUTTER_SOUND`로 선택(기본 OFF) | 필수 |
...
| 셔터음 | 앱이 내는 셔터음은 `MediaActionSound`로 우리가 직접 재생하는 것이고 끌 수 있다. CameraX는 서드파티 촬영 시 소리를 자동으로 넣지 않는다. 다만 **국내판 일부 기기는 펌웨어가 촬영음을 강제**할 수 있고, 그 소리는 앱에서 끌 수 없다 (강제 범위는 기기·펌웨어별로 다르며 S23 Ultra에서 실측 필요, TC-12) | 기본 무음(`Settings.PLAY_SHUTTER_SOUND=false`). 앱을 꺼도 소리가 나면 펌웨어 강제이며 앱 레벨 해결 불가 |
